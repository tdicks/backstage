...
test('an owner can reject a pending song request', function () {
    $owner = User::factory()->create();
    $requester = User::factory()->create();
    ...
    expect($songRequest->responded_by_user_id)->toBe($owner->id);
    expect($songRequest->song_id)->toBeNull();
    expect(Song::query()->where('set_id', $set->id)->count())->toBe(0);
});