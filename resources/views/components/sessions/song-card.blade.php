...
@php
    ...
    $canManageSongAttachments = $canManageSet || $song->slots->contains(fn ($slot) => (int) $slot->user_id === (int) auth()->id());
@endphp
...
<article
    ...
    x-data="sessionSongCard(@js([
        ...
        'attachmentsListUrl' => route('songs.attachments.index', $song),
        'attachmentsStoreUrl' => route('songs.attachments.store', $song),
        'canManageAttachments' => $canManageSongAttachments,
        'initialAttachmentCount' => $song->attachments_count ?? 0,
...
>
        ...
        <div class="min-w-0 flex-1">
            <h4 class="flex items-center gap-2 text-base font-semibold text-slate-900">
                ...
                <button
                    ...
                    title="Open song attachments"
                >
                    <x-heroicon-m-paper-clip class="h-4 w-4" aria-hidden="true" />
                    <span class="sr-only">Song has attachments</span>
                </button>
            </h4>
            @if ($song->notes)
    ...
    @if ($canManageSet && ! $setLocked)
        <div x-show="openEditSong" x-cloak x-transition.opacity.duration.150ms data-drag-blocking-modal data-modal-overlay class="fixed inset-0 z-40 bg-black/40" @click="openEditSong = false"></div>
        ...
        <div x-show="openAddSlot" x-cloak x-transition.opacity.duration.150ms data-drag-blocking-modal data-modal-overlay class="fixed inset-0 z-40 bg-black/40" @click="openAddSlot = false"></div>
