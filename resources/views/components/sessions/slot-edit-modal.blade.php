...
@if ($liveDashboard)
    <template x-teleport="body">
        <div x-show="openEditSlot" x-cloak @keydown.escape.window="closeEditSlotModal()">
            <div x-show="openEditSlot" x-transition.opacity.duration.150ms data-drag-blocking-modal data-modal-overlay class="fixed inset-0 z-40 bg-black/40" @click="closeEditSlotModal()"></div>
...
@elseif ($canManageSet && ! $setLocked)
    <div x-show="openEditSlot" x-cloak x-transition.opacity.duration.150ms data-drag-blocking-modal data-modal-overlay class="fixed inset-0 z-40 bg-black/40" @click="openEditSlot = false"></div>
