            ...
            <template x-for="session in currentAvailabilitySessions()" :key="`availability-row-${session.jam_session_id}`">
                <div class="rounded-lg border border-slate-200 bg-slate-50/95 p-4">
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        ...
                        <button type="button" @click="setAttendanceStatus(session, 'not_going')" x-bind:disabled="session.is_closed || isAttendanceSaving(session.jam_session_id) || session.my_status === 'not_going'" class="inline-flex items-center rounded-md border border-slate-300 bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition enabled:hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-50">Not Going</button>
